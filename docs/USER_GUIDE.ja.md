...
検索結果は10,000件で打ち切ります。読めない子ディレクトリは、不完全な結果を黙って返さずエラーとして通知します。
...
ローカル/SFTPコピーとアーカイブ展開中に `esc` を押すとキャンセル確認を開きます。ディレクトリ項目ごと、ファイル読み書きチャンクごとに中断を確認し、未完成の出力は可能な範囲で削除します。
...
設定ファイルは一時ファイルへ書き込んでから原子的に置き換えます。対応していない将来の `schemaVersion` や未知の項目を検出した場合は、元ファイルを書き換えず読み込みを停止します。`operation-failures.log` は5 MiBを超えると `.1` へローテーションします。
...
上の例はPOSIX shell向けです。初回生成されるサンプルは、利用中のPOSIX shell、PowerShell、`cmd`に合わせて選択されます。安全なクォート方法を判定できないshellでは、Windyは外部コマンドテンプレートの展開を拒否します。
